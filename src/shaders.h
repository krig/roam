...
static const char* chunk_fshader = "#version 130\n"
	...
	"}\n"
	"void main() {\n"
	"    vec4 tex = vec4(1, 1, 1, 1);\n"//texture(tex0, out_texcoord);\n"
	"    if (tex.w == 0) discard;\n"
	"    vec3 light = clamp(out_color.xyz + ((amb_light.xyz * out_color.w)*0.9 + 0.1), 0, 1);\n"
...
static const char* chunkalpha_fshader = "#version 130\n"
	...
	"}\n"
	"void main() {\n"
	"    vec4 tex = vec4(1, 1, 1, 0.5);\n"//texture(tex0, out_texcoord);\n"
	"    if (tex.w == 0) discard;\n"
	"    vec3 light = ((amb_light.xyz * out_color.w)*0.9 + 0.1);\n"
	"    vec3 base = tex.xyz * light.xyz;\n"
	"    vec3 fogged = fog(base, fog_color.xyz, out_depth, fog_color.w);\n"
